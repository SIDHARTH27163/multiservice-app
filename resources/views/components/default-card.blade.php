...
<article class="flex flex-col  pb-4 w-full bg-white  max-md:max-w-full  rounded-xl shadow-xl">
    ...
    <header class="flex gap-2.5 items-start self-start mt-4 leading-tight lg:text-base md:text-base sm:text-xs text-xs">
          ...
          <div class="px-2">
            <h2 class="mt-3 text-justify  lg:text-3xl md:text-3xl sm:text-base text-base lg:font-bold md:font-bold sm:font-semibold font-normal leading-7 max-md:mx-4  lg:mx-2 md:mx-2 sm:mx-1 mx-0 ">
                {{$title}}
              </h2>
              @if(!empty($description))
        <p class="mt-5 mx-3 text-xl leading-10 max-md:mt-10 max-md:max-w-full font-Robotoregular">
            {{ $description }}
        </p>
    @endif
          </div>
